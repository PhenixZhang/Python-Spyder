...
class ScrapyMyCSDN:
    ...
    def getOriginalArticalNums(self):
        ...
        if main_response.status_code == 200:
            ...
            mainpage_str = main_doc('div.data-info').text() #页面信息去除标签信息
            origin_position = mainpage_str.index('原创') #找到原创的位置
            end_position = mainpage_str.index('粉丝') #最终的位置,即原创底下是数字多少篇博文
            self.blog_nums = mainpage_str[origin_position+3:end_position-1]
...
#如何调用该类
mycsdn = ScrapyMyCSDN('qq_36936730') #初始化类 参数为博客名
...
#         break
#     blog_nums += mainpage_str[end_position + num]
# cur_blog_nums = int(blog_nums) #获得当前博客文章数量